...
mainM :: Monad m => (forall a. (m a -> IO a))
         -> Res m -> IO ()
mainM exec (GCJ gen) = do
  let f y = fmap (\x -> if fst x `elem` y then "" else snd x) . zip [1..]
      cc []     = return ("testInput", return (), putStr)
      cc (a:[]) = do h <- openFile (a++".out") WriteMode 
                     return (a, hClose h, \x -> putStr x >> hPutStr h x) 
  (inF,close,myP) <- cc =<< getArgs 
  (head:tail) <- lines <$> readFile inF
  (work,rest) <- (exec $ runStateT (replicateM (read head) gen) tail)
